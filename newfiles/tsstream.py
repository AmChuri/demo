...
def parsePATSection(filehandle, k):
    ...
    if (table_id != 0x0):
        print
        'Ooops! error in parsePATSection()!'
        return
    ...
    print
    'section_length = %d' % section_length
    ...
    print
    'section_number = %d, last_section_number = %d' % (section_number, last_section_number)
    ...
    while (length > 0):
        ...
        print
        'program_number = 0x%X' % program_number
        if (program_number == 0):
            print
            'network_PID = 0x%X' % program_map_PID
        else:
            print
            'program_map_PID = 0x%X' % program_map_PID
        length = length - 4;
        j += 4
...
def parsePMTSection(filehandle, k):
    ...
    print
    'program_number = %d' % program_number
    ...
    print
    'PCR_PID = 0x%X' % PCR_PID
    program_info_length = (local & 0xFFF)
    print
    'program_info_length = %d' % program_info_length
    ...
    while (n > 0):
        ...
        print
        'descriptor_tag = %d, descriptor_length = %d' % (descriptor_tag, descriptor_length)
    ...
    while (length > 0):
        ...
        print
        'stream_type = 0x%X, elementary_PID = 0x%X, ES_info_length = %d' % (stream_type, elementary_PID, ES_info_length)
        n = ES_info_length
        m = j + 5;
        while (n > 0):
            descriptor_tag = readFile(filehandle, m, 1)
...
def parseSITSection(filehandle, k):
    ...
    if (table_id != 0x7F):
        print
        'Ooops! error in parseSITSection()!'
        return
    ...
    print
    'transmission_info_loop_length = %d' % transmission_info_loop_length
    ...
    while (length > 0):
        ...
        print
        'service_id = %d, service_loop_length = %d' % (service_id, service_loop_length)
...
def parseTSMain(filehandle, packet_size, mode, pid, psi_mode, searchItem):
        ...
        while (True):
            ...
            if (adaptation_fieldc_trl == 0x2) | (adaptation_fieldc_trl == 0x3):
                ...
                if ((searchItem == "PCR") & ((flags >> 4) & 0x1)):
                    ...
                    print
                    'PCR packet, packet No. %d, PID = 0x%x, PCR_base = hi:0x%X lo:0x%X PCR_ext = 0x%X %s' \
            ...
            if (adaptation_fieldc_trl == 0x1) | (adaptation_fieldc_trl == 0x3):
                ...
                if ((PESstartCode & 0xFFFFFF00) == 0x00000100) & \
                    ...
                    print
                    'PES start, packet No. %d, PID = 0x%x, PTS_MSB24 = 0x%x PTS_hi = 0x%X, PTS_low = 0x%X' \
                    % (packetCount, PID, PTS_MSB24, PESPktInfo.PTS_hi, PESPktInfo.PTS_lo)

                    if (mode == 'ES'):
                        print
                        'packet No. %d,  ES PID = 0x%X,  Steam_ID = 0x%X,  AU_Type = %s' \
                    ...
                    if (table_id == 0x0):
                        ...
                        if (((searchItem == "FFF") & (mode == 'PAT')) | (searchItem == "PAT")):
                            ...
                            print
                            'pasing PAT Packet! packet No. %d, PID = 0x%X' % (packetCount, PID)
                        ...
                        if (((searchItem == "FFF") & (mode == 'PMT') & (PID == pid)) | (searchItem == "PMT")):
                            if ((psi_mode == 2) & (searchItem == "PMT")):
                                ...
                                if isUnique:
                                    ...
                                    continue
                            print
                            'pasing PMT Packet! packet No. %d, PID = 0x%X' % (packetCount, PID)
                        ...
                        if (((searchItem == "FFF") & (mode == 'SIT') & (PID == pid)) | (searchItem == "SIT")):
                            if ((psi_mode == 2) & (searchItem == "SIT")):
                                ...
                                if isUnique:
                                    ...
                                    continue
                            print
                            'pasing SIT Packet! packet No. %d, PID = 0x%X' % (packetCount, PID)
    ...
    for i in range(len(EntryPESPacketNumList)):
        print
        'TPI = 0x%x, PTS = 0x%x, EntryPESPacketNum = 0x%x' % (TPIList[i], PTSList[i], EntryPESPacketNumList[i])
